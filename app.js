(() => {
  ...
  const STORAGE_KEY = 'sevenam-retail-kpi-dashboard-v8';
  const BASE = { w:1536, h:1024 };
  const EXPORT = { w:2046, h:1364 }; // exact 3:2, ~2K width
  ...
  const ICON_ASSETS = {
    ...
    clock:'data:image/svg+xml;base64,',
    ...
    target:'data:image/svg+xml;base64,',
    ...
    comment:'data:image/svg+xml;base64,',
...
\tTổng\t7.010\t2.839,1\t2.713,5\t125,6\t219,5\t41%`;
  ...
  const els = {
    ...
    dateOverride:$('dateOverride'), exportBtn:$('exportBtn'), previewCanvas:$('previewCanvas'), canvasWrap:$('canvasWrap'),
  ...
  function parsePastedData(text){
    ...
    return {rows,totalRow,issues,targetMonth:reportMonth,reportYear,currentDay,totalDays,remainingDays,updateDate,closingDate,dateSource,timeProgress,systemTarget,systemActual,systemPct,systemCalcPct,systemDiff,systemStatus,systemGap,requiredPerDay,forecast,forecastPct,sorted,top5,groups};
  }
  ...
  function run(show=true){
    try{model=parsePastedData(els.dataInput.value);localStorage.setItem(STORAGE_KEY,JSON.stringify({data:els.dataInput.value,date:els.dateOverride.value}));renderValidation();renderPreview();if(show)toast('Đã đối chiếu và tạo Dashboard');}
    catch(err){model=null;els.validationBox.innerHTML=`<div class="err">✕ ${escapeHtml(err.message)}</div>`;clearCanvas();if(show)toast('Dữ liệu chưa hợp lệ');}
  }
  ...
  function drawDashboard(ctx,m){
    ...
    drawAssetIcon(ctx,'clock',61,y+50,65);
    text(ctx,'TIẾN ĐỘ THỜI GIAN',107,y+16,18.5,C.text,'700');
    fitText(ctx,fmtPct(m.timeProgress,1),195,y+43,248,51,C.purple,'700','center',41);
    fitText(ctx,`${m.currentDay}/${m.totalDays} NGÀY ĐÃ QUA`,327,y+91,268,14.8,C.muted,'700','right',12.8);
    progress(ctx,41,y+112,291,11,m.timeProgress,C.purple);

    // 2. Sales completion
    shadowCard(ctx,358,y,352,h,10);
    drawAssetIcon(ctx,'completion',396,y+50,62);
    text(ctx,'TỶ LỆ HOÀN THÀNH DOANH SỐ',439,y+17,16.5,C.text,'700');
    fitText(ctx,fmtPct(m.systemPct,1),542,y+45,298,52,C.greenDark,'700','center',42);
    fitText(ctx,`(${fmtNum(m.systemActual)} / ${fmtNum(m.systemTarget)})`,534,y+101,300,16.8,C.text,'700','center',13.8);

    // 3. Monthly target
    shadowCard(ctx,717,y,276,h,10);
    drawAssetIcon(ctx,'target',754,y+50,62);
    ...
    // 4 + 5. System completion and month-end forecast
    shadowCard(ctx,999,y,514,h,10);
    const halfW=257,leftCX=999+halfW/2,rightCX=999+halfW+halfW/2;
    drawAssetIcon(ctx,'sales',1029,y+49,60);
    text(ctx,'TỶ LỆ HOÀN THÀNH',1071,y+14,14.6,C.text,'700');
    text(ctx,'HỆ THỐNG',1071,y+34,14.6,C.text,'700');
    fitText(ctx,fmtPct(m.systemPct,1),leftCX+8,y+52,214,48,C.greenDark,'700','center',38);
    fitText(ctx,`${fmtPoint(m.systemDiff)} điểm % vs TG`,leftCX,y+101,220,17.7,m.systemStatus.color,'700','center',14.8);
    ...
    fitText(ctx,`≈ ${fmtNum(m.forecast)}`,rightCX,y+101,220,19,'#f06f12','700','center',15);
    ...
    m.sorted.forEach((r,i)=>{
      ...
      progress(ctx,xBar,ry+13,barW,12,r.pct,r.status.color,m.timeProgress);text(ctx,fmtPct(r.pct,0),678,ry+7,18,r.status.color,'700');circle(ctx,752,ry+18,5.8,r.status.color);fitText(ctx,r.status.label,766,ry+8,119,12.8,r.status.color,'700');
    });
    ...
    layouts.forEach((it,gi)=>{
      ...
      drawInsightIcon(ctx,it.g[1],949,gy+gh/2,35,it.g[3]);
    ...
    // LOWER RIGHT: two clearly separated modules — Schedule and GAP KPI.
    const lowY=825,lowH=96;shadowCard(ctx,rx,lowY,rw,lowH,10);
    const splitX=1208;
    line(ctx,splitX,lowY+9,splitX,lowY+87,'#c3c9ce',1.2);

    // Schedule module
    const sch=m.systemDiff, schColor=m.systemStatus.color, schTitle=sch<0?'BEHIND SCHEDULE':'AHEAD SCHEDULE';
    text(ctx,'SO VỚI TIẾN ĐỘ',1060,lowY+8,16.2,C.navy2,'700','center');
    drawAssetIcon(ctx,'schedule',950,lowY+63,42);
    fitText(ctx,schTitle,1073,lowY+39,190,13.2,schColor,'700','center',11.8);
    fitText(ctx,`${fmtPoint(sch)} điểm %`,1073,lowY+57,205,25,schColor,'700','center',20);

    // GAP module: title + two mini cells (GAP value / required per day)
    text(ctx,'GAP TO KPI',1360,lowY+8,16.2,C.orange,'700','center');
    const gapInnerTop=lowY+35, gapInnerBottom=lowY+88, gapSplit=1362;
    line(ctx,gapSplit,gapInnerTop,gapSplit,gapInnerBottom,'#d2d6da',1);
    drawAssetIcon(ctx,'gap',1242,lowY+63,40);
    fitText(ctx,fmtNum(m.systemGap),1310,lowY+52,91,25,C.orange,'700','center',19);
    text(ctx,'CẦN ĐẠT / NGÀY',1434,lowY+42,12.8,C.navy2,'700','center');
    fitText(ctx,fmt1(m.requiredPerDay),1434,lowY+60,126,22,C.navy2,'700','center',17);

    // FOOTER STATUS LEGEND — four equal segments, centered as one visual unit.
    rr(ctx,23,928,1490,62,9,'rgba(6,18,29,.78)','#607181',1);
    const legend=[
    ...
    const footerX=23,footerW=1490,seg=footerW/4,dotR=8,dotD=dotR*2;
    const labelSize=15.2,condSize=13.0,noteSize=12.0,gapDot=9,gapText=9;
    legend.forEach((l,i)=>{
      const sx=footerX+i*seg,center=sx+seg/2;
      if(i)line(ctx,sx,936,sx,982,'rgba(145,160,173,.25)',1);
      ...
      circle(ctx,startX+dotR,947,dotR,l[0]);
      ctx.save();ctx.textBaseline='middle';ctx.textAlign='left';ctx.fillStyle=l[0];ctx.font=`700 ${labelSize}px ${FONT_STACK}`;ctx.fillText(l[1],startX+dotD+gapDot,947);ctx.fillStyle='#f5f7f8';ctx.font=`700 ${condSize}px ${FONT_STACK}`;ctx.fillText(l[2],startX+dotD+gapDot+lw+gapText,947);ctx.restore();
      fitText(ctx,l[3],center,965,seg-30,noteSize,'#eef2f5','700','center',10.8);
    });
    fitText(ctx,'Ghi chú: Chênh lệch trạng thái = “Hoàn thành TG tháng” − tiến độ thời gian.',46,998,665,13.4,'#f0f3f5','700','left',11.6);line(ctx,736,997,736,1017,'#d4d7da',1.1);fitText(ctx,`Thời gian: ${m.currentDay}/${m.totalDays} ngày (${fmtPct(m.timeProgress,1)})`,770,998,292,13.4,'#f0f3f5','700','left',11.6);line(ctx,1087,997,1087,1017,'#d4d7da',1.1);fitText(ctx,`Dữ liệu chốt hết ngày ${dateVN(m.closingDate)}`,1117,998,374,13.4,'#f0f3f5','700','left',11.6);
  }
  ...
  async function exportPng(){if(!model){toast('Hãy phân tích dữ liệu trước');return;}const old=els.exportBtn.textContent;try{els.exportBtn.disabled=true;els.exportBtn.textContent='Đang xuất PNG…';const c=document.createElement('canvas');c.width=EXPORT.w;c.height=EXPORT.h;const ctx=c.getContext('2d',{alpha:false});const scale=EXPORT.w/BASE.w;ctx.setTransform(scale,0,0,scale,0,0);drawDashboard(ctx,model);const blob=await canvasToBlob(c);downloadBlob(blob,`KPI-He-Thong-Ban-Le-${isoDate(model.closingDate)}-2K.png`);toast('Đã xuất PNG 2K');}catch(e){console.error(e);toast('Xuất ảnh thất bại');}finally{els.exportBtn.disabled=false;els.exportBtn.textContent=old;}}
  ...
  els.exportBtn.addEventListener('click',async()=>{await iconsReady;return exportPng();});
